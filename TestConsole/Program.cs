...
Answer Answer = new Answer("Sun is yellow", false,110,2);
await ar.Add(Answer);
...
Certificate Certificate = new Certificate(1,"Some media templatepath",2, new CertificateToCourse());
...
Course Course = new Course("20 minutes", "course for banking",CourseType.Passed,null,new CourseToExam());
...
Exam Exam = new Exam("20 minutes", "Exam for banking", new List<ExamToQuestion>());
await er.Add(Exam);
...
Exam = await ec.CreateExamWithId("20 minutes", "Exam for banking",ids);
await er.Add(Exam);
Exam = await er.Get(Exam.Id);
Console.WriteLine(JsonConvert.SerializeObject(Exam));
...
Exam = await ec.CreateExamWithType("20 minutes", "Exam for banking", keyValues);
await er.Add(Exam);
Exam = await er.Get(Exam.Id);
Console.WriteLine(JsonConvert.SerializeObject(Exam));